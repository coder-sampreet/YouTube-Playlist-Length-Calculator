async function getPlaylistDuration() {
    ...
    if (!playlistId) {
        displayErrorMessage("Invalid playlist URL. Please enter a valid YouTube playlist link.");
        return;
    }
    ...
    try {
        ...
        if (!playlistResponse.ok || !playlistData.items || playlistData.items.length === 0) {
            displayErrorMessage("Unable to retrieve playlist details. Please check the playlist URL and try again.");
            return;
        }
